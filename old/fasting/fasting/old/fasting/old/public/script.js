...
function recoverAccount() {
    ...
    .then(data => {
        if (data.error) {
            ...
            result.innerHTML = `<span style="color:#25d366;">✅ كلمة السر: <strong>${data.password}</strong></span>`;
        }
    });
}
...
function openChatWith(friendId) {
    ...
    fetch(`/blocked-messages/${myId}/${friendId}`)
    .then(res => res.json())
    .then(data => {
        const messages = data.messages || [];
...
function adminResetPassword() {
    ...
    fetch('/admin/reset-password', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            adminKey: "MySecretAdminKey2026",   // نفس المفتاح اللي في السيرفر
            userId: userId,
    ...
    .then(data => {
        if (data.error) {
            ...
            modalSuccess(data.message || 'تم تغيير كلمة السر بنجاح');
            document.getElementById('adminUserId').value = '';
            document.getElementById('adminNewPassword').value = '';
        }
    })
    .catch(() => modalError('حدث خطأ في الاتصال'));
}
